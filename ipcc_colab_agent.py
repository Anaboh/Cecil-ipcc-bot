...
class IPCCLLMAgent:
    ...
    def __init__(self):
        ...
        # Added DeepSeek to models and set as default
        self.llm_models = {
            'deepseek': {'name': 'DeepSeek', 'provider': 'DeepSeek'},  # Set as first/default
        ...
        self.deepseek_key = None  # For DeepSeek
        ...
        print(f"  DeepSeek: {'✅ Available' if self.deepseek_key else '❌ Not available'}")
...
- Note uncertainties and ranges in projections"""
        ...
        try:
            ...
            elif model == 'deepseek' and self.deepseek_key:
        ...
        # Safety net: Return mock response if all else fails
        print("⚠️ Unexpected code path reached, returning mock response")
        user_message = messages[-1]['content'] if messages else ""
        return self.get_mock_response(user_message, 'all')
    ...
    with gr.Blocks(
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                # Set DeepSeek as default model
                model_choice = gr.Dropdown(
                    choices=list(IPCCLLMAgent().llm_models.keys()),
                    value="deepseek",  # Set DeepSeek as default
                    label="AI Model",
                ...
                • GEMINI_API_KEY<br>
                • <strong>DEEPSEEK_API_KEY</strong> (default model)<br><br>
                ...
                {'✅' if os.getenv('DEEPSEEK_API_KEY') else '❌'} <strong>DeepSeek</strong>: {'Available' if os.getenv('DEEPSEEK_API_KEY') else 'No API Key'}<br>
